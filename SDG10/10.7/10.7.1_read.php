  ...
  <div class="container">
    ...
    <table>
      <thead>
        <tr>
          <th>ID</th>
          <th>total_refugee_students_male</th>
          <th>total_refugee_students_female</th>
          <th>student_refugee_percentage</th>
          <th colspan="2">Actions</th>
        </tr>
      </thead>
      <tbody>
        <?php
        $query = "SELECT * FROM `10.7.1 Proportion of refugee students`";             // SQL query to fetch all table data
        ...
        while ($row = mysqli_fetch_assoc($view_records)) {
          ...
          echo "<tr>";
          echo "<td>{$id}</td>";
          echo "<td>{$total_refugee_students_male}</td>";
          echo "<td>{$total_refugee_students_female}</td>";
          echo "<td>{$student_refugee_percentage}</td>";
          
          echo "<td><a href='10.7.1_update.php?id={$id}' class='btn btn-edit'>Edit</a></td>";
          echo "<td><a href='10.7.1_delete.php?delete={$id}' class='btn btn-delete'>Delete</a></td>";
          echo "</tr>";
        }
        ?>
      </tbody>
