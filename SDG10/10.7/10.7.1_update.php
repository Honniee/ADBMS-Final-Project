...
$query = "SELECT * FROM `10.7.1 Proportion of refugee students` WHERE id = $id";
$view = mysqli_query($connect, $query);

if ($view) {
    while ($row = mysqli_fetch_assoc($view)) {
        $id = $row['id'];
        $total_refugee_students_male = $row['total_refugee_students_male'];
        $total_refugee_students_female = $row['total_refugee_students_female'];
        $student_refugee_percentage = $row['student_refugee_percentage'];
    }
} else {
...
if (isset($_POST['update'])) {
    ...
    $query = "UPDATE `10.7.1 Proportion of refugee students` SET 
        total_refugee_students_male = '{$total_refugee_students_male}', 
        total_refugee_students_female = '{$total_refugee_students_female}', 
        student_refugee_percentage = '{$student_refugee_percentage}' 
        WHERE id = $id";
...
<div class="form-container">
    <form action="" method="POST">
        <div class="form-group">
            <label for="total_refugee_students_male">Total Refugee Students (Male):</label>
            <input type="number" name="total_refugee_students_male" id="total_refugee_students_male" value="<?php echo isset($total_refugee_students_male) ? $total_refugee_students_male : ''; ?>" required>
        </div>

        <div class="form-group">
            <label for="total_refugee_students_female">Total Refugee Students (Female):</label>
            <input type="number" name="total_refugee_students_female" id="total_refugee_students_female" value="<?php echo isset($total_refugee_students_female) ? $total_refugee_students_female : ''; ?>" required>
        </div>

        <div class="form-group">
            <label for="student_refugee_percentage">Student Refugee Percentage:</label>
            <input type="text" name="student_refugee_percentage" id="student_refugee_percentage" value="<?php echo isset($student_refugee_percentage) ? $student_refugee_percentage : ''; ?>" required>
        </div>
