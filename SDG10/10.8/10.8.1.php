...
<body>
  ...
  10.8.1 Non Discriminatory Admissions Policy
  <div class="point-system" style="margin-left: auto; padding-top: 10px;">
  <h4>Pointing System:
    <?php
      $policy_query = "SELECT total_number_of_non_discriminatory_admission_policy FROM `10.8.1 non discriminatory admissions policy`;";
      $policy_result = mysqli_query($connect, $policy_query);
      $policy_row = mysqli_fetch_assoc($policy_result);
      $policy_count = $policy_row['total_number_of_non_discriminatory_admission_policy'];
      
      $ppa_query = "SELECT COUNT(*) c FROM `10.8.1 non discriminatory admissions policy`;";
      ...
      echo $total_points;
      // Get the current total_points value for id=2
$sql = "SELECT total_points FROM total WHERE id=2;";
...
if ($total_points != $current_total_points) {
    ...
    if ($current_total_points == 0) {
        $sql = "INSERT INTO total(id, total_points) VALUES(2, '{$total_points}')";
    }
    // If there is a current value, update the existing record
    else {
        $sql = "UPDATE total SET total_points='{$total_points}' WHERE id=2;";
    }
    ...
    <?php 
        ...
        if(isset($_POST['save'])){
            $id = $_POST['id'];
            $total_number_of_non_discriminatory_admission_policy = $_POST['total_number_of_non_discriminatory_admission_policy'];
            ...
            if($id) {
                // Update 
                $query = "UPDATE `10.8.1 Non discriminatory admissions policy` SET total_number_of_non_discriminatory_admission_policy='$total_number_of_non_discriminatory_admission_policy', total_number_of_PPAs_implemented_in_accordance_to_the_policy='$total_number_of_PPAs_implemented_in_accordance_to_the_policy', title_of_the_PPA='$title_of_the_PPA', short_description_of_the_PPA='$short_description_of_the_PPA', total_cost='$total_cost', evidence='$evidence' WHERE id=$id";
            } else {
                // Create
                $query = "INSERT INTO `10.8.1 Non discriminatory admissions policy` (total_number_of_non_discriminatory_admission_policy, total_number_of_PPAs_implemented_in_accordance_to_the_policy, title_of_the_PPA, short_description_of_the_PPA, total_cost, evidence) VALUES ('$total_number_of_non_discriminatory_admission_policy', '$total_number_of_PPAs_implemented_in_accordance_to_the_policy', '$title_of_the_PPA', '$short_description_of_the_PPA', '$total_cost', '$evidence')";
            }
        ...
        if(isset($_GET['delete'])){
            $id = $_GET['delete'];
            $delete_query = "DELETE FROM `10.8.1 Non discriminatory admissions policy` WHERE id=$id";
        ...
        if(isset($_GET['edit'])){
            $id = $_GET['edit'];
            $edit_query = "SELECT * FROM `10.8.1 Non discriminatory admissions policy` WHERE id=$id";
    ...
    <div id="form-container">
        ...
        <form action="" method="post">
            ...
            <div>
                <label for="total_number_of_non_discriminatory_admission_policy">Total Number of Non Discriminatory Admission Policy:</label>
                <input type="number" name="total_number_of_non_discriminatory_admission_policy" value="<?php echo isset($edit_data['total_number_of_non_discriminatory_admission_policy']) ? $edit_data['total_number_of_non_discriminatory_admission_policy'] : ''; ?>" required>
            </div>
            <div>
                <label for="total_number_of_PPAs_implemented_in_accordance_to_the_policy">Total Number Of PPAs Implemented In Accordance To The Policy:</label>
                <input type="number" name="total_number_of_PPAs_implemented_in_accordance_to_the_policy" value="<?php echo isset($edit_data['total_number_of_PPAs_implemented_in_accordance_to_the_policy']) ? $edit_data['total_number_of_PPAs_implemented_in_accordance_to_the_policy'] : ''; ?>" required>
            </div>
            <div>
                <label for="title_of_the_PPA">Title of the PPA:</label>
                <input type="text" name="title_of_the_PPA" value="<?php echo isset($edit_data['title_of_the_PPA']) ? $edit_data['title_of_the_PPA'] : ''; ?>" required>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="evidence" value="<?php echo isset($edit_data['evidence']) ? $edit_data['evidence'] : ''; ?>" required>
            </div>
            <div>
        ...
        <?php
            $fetchquery = "SELECT * FROM `10.8.1 Non discriminatory admissions policy`";
            ...
            if(mysqli_num_rows($fetch_data) > 0){
                echo "<table>
                        <tr>
                            <th>ID</th>
                            <th>Total Non-Discriminatory Policies</th>
                            <th>Total PPAs Implemented</th>
                            <th>Title of PPA</th>
                ...
                while($row = mysqli_fetch_assoc($fetch_data)){
                    echo "<tr>
                            <td>{$row['id']}</td>
                            <td>{$row['total_number_of_non_discriminatory_admission_policy']}</td>
                            <td>{$row['total_number_of_PPAs_implemented_in_accordance_to_the_policy']}</td>
