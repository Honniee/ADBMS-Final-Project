...
<body>
  ...
  <?php
      ...
      $query = "SELECT * FROM `10.8.6 support for underrepresented groups` WHERE id = {$userid}";
      $view_user = mysqli_query($connect, $query);
      while ($row = mysqli_fetch_assoc($view_user)){
        $id = $row['id'];
        $total_number_PPAs_to_support_underrepresented_groups = $row['total_number_PPAs_to_support_underrepresented_groups'];
      ...
      if(isset($_POST['update'])){
        ...
        $query = "UPDATE `10.8.6 support for underrepresented groups` SET total_number_PPAs_to_support_underrepresented_groups = '{$total_number_PPAs_to_support_underrepresented_groups}',title_of_the_ppa = '{$title_of_the_ppa}', short_description = '{$short_description}', total_cost = '{$total_cost}', fund_source = '{$fund_source}' WHERE id = $userid";
    ...
    <div>
      <form action = "" method = "post">
        <div>
          ...
          <input class = 'type' type="text" name = "total_number_PPAs_to_support_underrepresented_groups" value = "<?php echo $total_number_PPAs_to_support_underrepresented_groups ?>">
        </div>
        <div>
        ...
        <div>
          ...
          <input class = 'type' type="textbox" name = "short_description" value = "<?php echo $short_description ?>">
        </div>
        <div>
