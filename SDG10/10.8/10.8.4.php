...
<head>
  ...
  <link rel = "stylesheet" href="../styles/10.8.css">
    <title>SDG 10.8.4 Anti-Discrimination Policies</title>
    <style>
       body {
...
10.8.4 Anti-Discrimination Policies
<div class="point-system" style="margin-left: auto; padding-top: 10px;">
  <h4>Pointing System:
  <?php
    $policy_query = "SELECT COUNT(*) c FROM `10.8.4 anti-discrimination policies` WHERE total_number_of_PPAs_implemented_in_accordance_to_the_policy = 1;";
    $policy_result = mysqli_query($connect, $policy_query);
    $policy_row = mysqli_fetch_assoc($policy_result);
    $policy_points = $policy_row['c'] == 1 ? 2 : 0;
    
    $ppa_query = "SELECT COUNT(*) c FROM `10.8.4 anti-discrimination policies` WHERE total_number_of_PPAs_implemented_in_accordance_to_the_policy = 1;";
    $ppa_result = mysqli_query($connect, $ppa_query);
    $ppa_row = mysqli_fetch_assoc($ppa_result);
    $ppa_points = $ppa_row['c'] * 7 / 2;
    
    $total_points = $policy_points + $ppa_points;
    ...
    echo $total_points;
    // Get the current total_points value for id=4
$sql = "SELECT total_points FROM total WHERE id=4;";
...
if ($total_points != $current_total_points) {
    ...
    if ($current_total_points == 0) {
        $sql = "INSERT INTO total(id, total_points) VALUES(4, '{$total_points}')";
    }
    // If there is a current value, update the existing record
    else {
        $sql = "UPDATE total SET total_points='{$total_points}' WHERE id=4;";
    }
  ...
  ?>
    ...
    <?php 
        ...
        if(isset($_POST['save'])){
            ...
            if($id) {
                // Update 
                $query = "UPDATE `10.8.4 Anti-discrimination policies` SET total_number_of_PPAs_implemented_in_accordance_to_the_policy='$total_number_of_PPAs_implemented_in_accordance_to_the_policy', title_of_the_ppa='$title_of_the_ppa', short_description='$short_description', total_cost='$total_cost', fund_source='$fund_source', evidence='$evidence' WHERE id=$id";
            } else {
                // Create
                $query = "INSERT INTO `10.8.4 Anti-discrimination policies` (total_number_of_PPAs_implemented_in_accordance_to_the_policy, title_of_the_ppa, short_description, total_cost, fund_source, evidence) VALUES ('$total_number_of_PPAs_implemented_in_accordance_to_the_policy', '$title_of_the_ppa', '$short_description', '$total_cost', '$fund_source', '$evidence')";
            }
        ...
        if(isset($_GET['delete'])){
            $id = $_GET['delete'];
            $delete_query = "DELETE FROM `10.8.4 Anti-discrimination policies` WHERE id=$id";
        ...
        if(isset($_GET['edit'])){
            $id = $_GET['edit'];
            $edit_query = "SELECT * FROM `10.8.4 Anti-discrimination policies` WHERE id=$id";
    ...
    <div id="form-container">
        ...
        <form action="" method="post">
            ...
            <div>
                ...
                <input type="text" name="short_description" value="<?php echo isset($edit_data['short_description']) ? $edit_data['short_description'] : ''; ?>" required>
            </div>
            <div>
        ...
        <?php
            $fetchquery = "SELECT * FROM `10.8.4 Anti-discrimination policies`";
