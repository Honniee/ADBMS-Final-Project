...
<body>
  <?php
    ...
    {
      if (isset($_POST['total_number_of_access_schemes_for_PWDs']) && is_numeric($_POST['total_number_of_access_schemes_for_PWDs'])) {
        $total_number_of_access_schemes_for_PWDs = $_POST['total_number_of_access_schemes_for_PWDs'];
      } else {
      ...
      $total_number_of_access_schemes_for_PWDs = $_POST['total_number_of_access_schemes_for_PWDs'];
      $title_of_the_access_scheme = $_POST['title_of_the_access_scheme'];
      ...
      if (!is_numeric($total_number_of_access_schemes_for_PWDs)) {
        echo "<script type = 'text/javascript'>alert('Error: Total number of PPAs must be a number.')</script>";
        exit();
      }

      $query = "INSERT INTO `10.8.9 disability access scheme`(total_number_of_access_schemes_for_PWDs, title_of_the_access_scheme, short_description, total_cost, fund_source) VALUES('{$total_number_of_access_schemes_for_PWDs}','{$title_of_the_access_scheme}','{$short_description}','{$total_cost}','{$fund_source}')";
  ...
  10.8.9 Disability access scheme
  <div class="point-system" style="margin-left: auto; padding-top: 10px;">
  <h4>Pointing System:
    <?php
      $query = "SELECT COUNT(*) c FROM `10.8.9 disability access scheme`;";
      ...
      echo $total_points;
      // Get the current total_points value for id=9
$sql = "SELECT total_points FROM total WHERE id=9;";
...
if ($total_points != $current_total_points) {
    ...
    if ($current_total_points == 0) {
        $sql = "INSERT INTO total(id, total_points) VALUES(9, '{$total_points}')";
    }
    // If there is a current value, update the existing record
    else {
        $sql = "UPDATE total SET total_points='{$total_points}' WHERE id=9;";
    }
  ...
  <div class = "content">
    <form action="" method="post">
      <div class = "line">
        <label for = "ppa">Total number of access schemes for PWDs: </label>
        <input class="type1" type="textbox" placeholder="Type here" name = "total_number_of_access_schemes_for_PWDs">
      </div>
      <div class = "line">
        <label for = "ppa">Title of the access scheme: </label>
        <input class="type2" type="textbox" placeholder="Type here" name = "title_of_the_access_scheme">
      </div>
      <div class = "line">
      ...
      <div class = "submit-button-wrapper">
        ...
        <div class = "view-button"><a href = "10.8.9read.php">View</a></div>
      </div>
      <div>
