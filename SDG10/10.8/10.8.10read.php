...
<body>
  ...
  <table>
    <thead>
      ...
      <tr>
        <th>ID</th>
        <th>Total number of disability accommodation policy</th>
        <th>Total number of PPAs implemented in accordance to the policy</th>
      ...
      <tr>
      <?php
        $query = "SELECT * FROM `10.8.10 disability accommodation policy`;";
        ...
        while($row = mysqli_fetch_assoc($view_users)){
          ...
          echo "<td> {$total_number_of_disability_accomodation_policy}</td>";
          echo "<td> {$total_number_of_PPAs_implemented_in_accordance_to_the_policy}</td>";
          echo "<td> {$title_of_the_ppa}</td>";
          echo "<td> {$short_description}</td>";
          echo "<td> {$total_cost}</td>";
          echo "<td> {$fund_source}</td>";

          echo "<td> <a href = '10.8.10update.php?user_id={$id}' class = 'button2'> EDIT </a></td>";
          echo "<td> <a href = '10.8.10delete.php?delete={$id}' class = 'button3'> DELETE </a></td>";
        }
      ?>
      </tr>
  </table>
  <div>
    <button class = "back-button" onclick = "window.location.href= '10.8.10.php';">&laquo; Back</button>
  </div>
