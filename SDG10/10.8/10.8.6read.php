...
<body>
  ...
  <table>
    <thead>
      ...
      <tr>
      <?php
        $query = "SELECT * FROM `10.8.6 support for underrepresented groups`;";
        ...
        while($row = mysqli_fetch_assoc($view_users)){
          ...
          echo "<td> {$total_number_PPAs_to_support_underrepresented_groups}</td>";
          echo "<td> {$title_of_the_ppa}</td>";
          echo "<td> {$short_description}</td>";
          echo "<td> {$total_cost}</td>";
          echo "<td> {$fund_source}</td>";

          echo "<td> <a href = '10.8.6update.php?user_id={$id}' class = 'button2'> EDIT </a></td>";
          echo "<td> <a href = '10.8.6delete.php?delete={$id}' class = 'button3'> DELETE </a></td>";
        }
      ?>
      </tr>
  </table>
  <div>
    <button class = "back-button" onclick = "window.location.href= '10.8.6.php';">&laquo; Back</button>
  </div>
