...
<body>
  <?php
    ...
    {
      if (isset($_POST['total_number_of_the_support_service']) && is_numeric($_POST['total_number_of_the_support_service'])) {
      ...
      if (!is_numeric($total_number_of_the_support_service)) {
        echo "<script type = 'text/javascript'>alert('Error: Total number of PPAs must be a number.')</script>";
        exit();
      }

      $query = "INSERT INTO `10.8.8 disability support services for students and employees`(total_number_of_the_support_service, title_type_of_the_support_service, short_description, total_cost, fund_source) VALUES('{$total_number_of_the_support_service}','{$title_type_of_the_support_service}','{$short_description}','{$total_cost}','{$fund_source}')";
  ...
  10.8.8 Disability support services for students and employees
  <div class="point-system" style="margin-left: auto; padding-top: 10px;">
  <h4>Pointing System:
    <?php
      $query = "SELECT COUNT(*) c FROM `10.8.8 disability support services for students and employees`;";
      ...
      echo $total_points;
      // Get the current total_points value for id=8
$sql = "SELECT total_points FROM total WHERE id=8;";
...
if ($total_points != $current_total_points) {
    ...
    if ($current_total_points == 0) {
        $sql = "INSERT INTO total(id, total_points) VALUES(8, '{$total_points}')";
    }
    // If there is a current value, update the existing record
    else {
        $sql = "UPDATE total SET total_points='{$total_points}' WHERE id=8;";
    }
  ...
  <div class = "content">
    <form action="" method="post">
      <div class = "line">
        <label for = "ppa">Total number of support services for PWDs: </label>
        <input class="type1" type="textbox" placeholder="Type here" name = "total_number_of_the_support_service">
      </div>
      <div class = "line">
        <label for = "ppa">Title/type of the support service: </label>
        <input class="type2" type="textbox" placeholder="Type here" name = "title_type_of_the_support_service">
      </div>
      <div class = "line">
      ...
      <div class = "submit-button-wrapper">
        ...
        <div class = "view-button"><a href = "10.8.8read.php">View</a></div>
      </div>
      <div>
